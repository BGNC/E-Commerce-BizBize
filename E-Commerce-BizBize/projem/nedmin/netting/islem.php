...
if (@$_GET['sikayetsil']=="ok") {
	...
	$sil=$db->prepare("DELETE from sikayet where mesaj_id=:mesaj_id");
	$kontrol=$sil->execute(array(
		'mesaj_id' => $_GET['sikayet_id']
	));

	if ($kontrol) {

		Header("Location:../production/sikayet.php?durum=ok");
		exit;

	} else {

		Header("Location:../production/sikayet.php?durum=no");
		exit;
	}
