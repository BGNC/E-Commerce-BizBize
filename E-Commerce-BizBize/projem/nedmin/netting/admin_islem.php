...
if(isset($_POST['sikayetet']))
{
	...
	$sikayetekle=$db->prepare("INSERT INTO sikayet SET
		name 		=:name,
		email 		=:email,
		konu 		=:konu,
		mesaj 		=:mesaj
		");
	$ekle=$sikayetekle->execute(array(
		'name'	 	=>$name,
		'email'		=>$email,
		'konu'		=>$konu,
		'mesaj'		=>$mesaj
	...
	{
		Header("Location:../../contact.php?durum=hata");
		exit;
	}

}

if (isset($_POST['bloglogoduzenle'])) {
	...
	if ($_FILES['blog_resim']['size']>1048576) {
		...
		Header("Location:../production/blog-onay-islemleri.php?durum=dosyabuyuk");
		exit;
	...
	if (in_array($ext, $izinli_uzantilar) === false) {
		echo "Bu uzantı kabul edilmiyor";
		Header("Location:../production/blog-onay-islemleri.php?durum=formathata");
	...
	if ($update) {
		...
		Header("Location:../production/blog-onay-islemleri.php?durum=ok");
		exit;

	} else {

		Header("Location:../production/blog-onay-islemleri.php?durum=no");
		exit;
	}

}

if (isset($_POST['blogonaykayit'])) {
	



	$bloguncelle=$db->prepare("UPDATE blog SET
	...
	$update=$bloguncelle->execute(array(
	...
	if ($update) {
		...
		Header("Location:../production/bloglar.php?durum=ok");
		exit;


	} else {

		Header("Location:../production/bloglar.php?durum=no");
		exit;
	}
